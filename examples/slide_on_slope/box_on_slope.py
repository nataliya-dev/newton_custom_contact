...
class BoxOnSlopeExample(BaseSlide):
    def __init__(self, viewer=None, args=None):
        super().__init__(viewer, args)
        self.example_name = "box_on_slope"
    ...
    def add_object_on_slope(self, builder, type):
        ...
        if type == "morphit":
            ...
            hx = hy = hz = 0.1
            center, rot = get_object_on_slope_initial_pose(
                slope=self.slope, up_slope_offset=up_slope_offset, object_bottom_offset_z=hz)
        ...
        if type == "mesh":
            ...
            builder = self.box.add_morphit_spheres(builder, json_adrs=self.args.morphit_json)
    ...
    def calculate_analytical_state(self, t):
        x0 = self.box.pos0
        rot0 = self.box.rot0
        return obj_on_slope_solution(x0=x0, rot0=rot0, mu=self.mu, t=t, slope_rot=self.slope.rot0, m=self.box.mass)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to JSON file for morphit slope representation (only for MRXPBD with sphere slope)"
    )
    ...
    example = BoxOnSlopeExample(viewer, args=args)
