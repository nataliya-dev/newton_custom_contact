...
class CollisionPipeline:
    ...
    def collide(
        ...
        *,
        ...
        # Operates on narrow-phase contacts only; runs BEFORE CSLC writes so
        # rigid_contact_count is still the pre-CSLC narrow-phase count.
        ...
        # Deterministic sort runs over narrow-phase contacts only — must happen
        # BEFORE CSLC writes to its reserved slots at cslc_contact_offset,
        # otherwise the sort would cover the uninitialized gap between the
        # narrow-phase count and cslc_contact_offset.
        ...
        # this frame, carrying the sticky history forward).  All matcher
        # operations work on rigid_contact_count, which is still the
        # pre-CSLC narrow-phase count at this point.
        ...
        # CSLC contact generation — writes to pre-allocated slots at the end
        # of the contacts buffer (no atomic_add), then extends
        # rigid_contact_count to cover those slots.  Runs LAST so the
        # contact-matcher / sorter only see narrow-phase contacts; CSLC
        # contacts are deterministic by lattice topology and need no
